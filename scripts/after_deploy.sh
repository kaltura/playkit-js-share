...
for i in {1..3}; do
  ...
  HTTP_CODE=$(curl -k -d "{'name': $packageName, 'npm_name': $repoName,  'version':$currentVersion, 'source':'npm', 'schema_type': 'playerV3OvpVersions', 'update_schema': $UPDATE_SCHEMA}" -H "Content-Type: application/json" --silent --output /dev/stderr --write-out "%{http_code}" --fail -X POST https://jenkins-central.prod.ovp.kaltura.com/generic-webhook-trigger/invoke?token=$1)
  STATUS_CODE=$?
